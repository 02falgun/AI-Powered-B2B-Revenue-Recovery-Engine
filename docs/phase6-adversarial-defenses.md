...
- **Defense Mechanism**: **Guardrail A (`guardrailCheckOverAmount`)** in [`src/lib/policy.ts`](src/lib/policy.ts) compares `approvedAmountPaise` against the DB-sourced `outstandingAmountPaise`. Any excess strictly triggers `HUMAN_REVIEW` with `GUARDRAIL_A_OVER_AMOUNT`.
...
- **Defense Mechanism**: **Guardrail B (`guardrailCheckNonPositiveAmount`)** in [`src/lib/policy.ts`](src/lib/policy.ts) rejects `approvedAmountPaise <= 0`, routing immediately to `HUMAN_REVIEW` with `GUARDRAIL_B_NON_POSITIVE_AMOUNT`.
...
- **Defense Mechanism**:
  1. **Sanitizer Defense**: `validateAndSanitizeExtraction()` in [`src/lib/ai-schema.ts`](src/lib/ai-schema.ts) detects non-INR currency keywords (`USD`, `EUR`, `GBP`, `$`, `€`, `£`) and sets `promisedAmountInr = null` and `promisedAmountPaise = null`.
  2. **Guardrail H Defense**: `guardrailCheckCurrencyAndPercentageAmbiguity()` in [`src/lib/policy.ts`](src/lib/policy.ts) flags currency ambiguity and yields `HUMAN_REVIEW` with `GUARDRAIL_H_CURRENCY_AMBIGUITY`.
...
- **Defense Mechanism**: `validateAndSanitizeExtraction()` in [`src/lib/ai-schema.ts`](src/lib/ai-schema.ts) strictly validates percentage commitments within `0 < percentValue <= 100`. Any percentage exceeding 100% is ignored and leaves `promisedAmountInr` as `null`, forcing `HUMAN_REVIEW`.
...
- **Defense Mechanism**: **Guardrail G (`guardrailCheckAuthoritativeInvoice`)** in [`src/lib/policy.ts`](src/lib/policy.ts) enforces that invoice facts injected from the authoritative DB record govern all calculations. External text references inside email bodies are ignored.
...
- **Defense Mechanism**:
  1. **Untrusted Data Isolation**: System prompt ([`src/lib/ai-prompt.ts`](src/lib/ai-prompt.ts)) explicitly isolates buyer email text inside raw data tags and instructs the model that instructions inside email body are `DATA`, never commands.
